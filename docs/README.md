...
Far поддерживает запуск *консольных алиасов* (псевдонимов), однако об удобстве их создания, загрузки и редактирования мы должны позаботиться сами.
Решению этой задачи и посвящён предлагаемый набор скриптов.
...
[Консольные алиасы](https://learn.microsoft.com/windows/console/console-aliases), также известные как *“макросы [DOSKEY](https://learn.microsoft.com/windows-server/administration/windows-commands/doskey)”*,
это общеизвестный, но в целом недооценённый инструмент работы.
...
Однако (и это определяющее преимущество) в отличии от батников, через алиасы DOSKEY доступны и команды фара
(`chdir`, `set`), а посредством префиксов доступны и команды плагинов (например *FAR Commands*, *Temporary Panel*).  
Через префикс `lua:` можно использовать даже макросы!

Примеры алиасов: [alias.sample.doskey](https://github.com/FarManagerLegacy/Alias/blob/main/alias.sample.doskey).
...
Раньше я использовал простой макрос для загрузки макросов при старте: <https://forum.farmanager.com/viewtopic.php?p=117329#p117329>.
...
- По умолчанию в качестве файла алиасов используется `alias.doskey`, который должен быть расположен рядом со скриптом.
  ...
  Это позволяет использовать ассоциацию на маску “**\*.doskey**”: `DOSKEY/MACROFILE="!\!.!"`:
  так можно иметь разные файлы с алиасами, и загружать их по мере надобности.
  </details>
...
- Формат файла: типичный ini-файл, разбитый на секции вида `[far.exe]`, `[cmd.exe]`.
  ...
  - Позволяют задавать алиасы раздельно для разных приложений.
    Например те алисы, которые имеют смысл только в фаре можно разместить в секции `[far.exe]`,
    а все прочие - в `[cmd.exe]` (фар понимает и те и другие).
    Также алиасы можно использовать в некоторых других приложениях, например `[nslookup.exe]`.
  - При большом количестве алиасов могут быть [проблемы](http://forum.farmanager.com/viewtopic.php?p=41492#p41492),
    разделение на 2 секции помогает.
  - Наличие секций позволяет *Colorer*’у распознавать и раскрашивать файл как INI
    (этого можно было достичь дав файлу имя с соответствующим расширением)
...
- Имя и путь к файлу алиасов можно переназначить изменяя опции в начале скрипта.  
  Чтобы после каждого обновления скрипта не приходилось вручную восстанавливать изменённые значения опций
  рекомендуется использовать возможности **cfgscript** из пакета [ScriptsBrowser](https://forum.farmanager.com/viewtopic.php?f=15&t=10418).
